...
extension UIColor {
    ...
    class var main: UIColor {
        return UIColor(rgb: 0x2c4f60)
    }
    
    class var lightMain: UIColor {
        return UIColor(rgb: 0xB8C2DA)
    }
    
    class var secondary: UIColor {
        return UIColor(rgb: 0xc93f45)
    }
    ...
    func loadImageWithUrl (url: String) {
        ...
        self.kf.setImage(with: URL(string: url),
                         ...
                         progressBlock: nil) { (image, _, _, _) in
                            ...
                            self.image = #imageLiteral(resourceName: "user")
        }
    }
...
struct Appearance {
    static func setGlobalAppearance() {
        ...
        UINavigationBar.appearance().tintColor = #colorLiteral(red: 0.9510119557, green: 0.9585197568, blue: 0.9621108174, alpha: 1)
...
extension UIViewController {
    func setUINavigationBarAppearance() {
        ...
        gradient.colors = [UIColor.main.cgColor, UIColor.main.cgColor]
...
extension String {
    ...
    var apiDateFormatting: Date {
        let dateSub = self[0...9]
        let timeSub = self[11...18]
        let formatter = DateFormatter()
        formatter.dateFormat = "YYYY-MM-DD HH:MM:ss"
        return formatter.date(from: "\(dateSub) \(timeSub)")!
    }
    var apiDateFormattingTime: Date {
        let timeSub = self[11...18]
        print("timeSub \(timeSub)")
        let formatter = DateFormatter()
        formatter.dateFormat = "HH:mm:ss"
        return formatter.date(from: timeSub)!
    }
...
extension UIViewController {
    ...
    func showPromptMessage(title: String, message: String, completionHandler: @escaping (Bool) -> ()) {
        let alertController = UIAlertController(title: title, message: message, preferredStyle: UIAlertControllerStyle.alert)
        
        let okAction = UIAlertAction(title: NSLocalizedString("Cancel", comment: ""), style: .cancel) { (action) -> Void in
            completionHandler(false)
        }
        let dismissAction = UIAlertAction(title: NSLocalizedString("OK", comment: ""), style: .default) { (action) -> Void in
